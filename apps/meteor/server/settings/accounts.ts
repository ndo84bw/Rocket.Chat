...
export const createAccountSettings = () =>
	settingsRegistry.addGroup('Accounts', async function () {
		...
		await this.section('Accounts_StatusVisibility', async function () {
			...
			await this.add('Accounts_StatusVisibility_DefaultRoles', '', {
				type: 'string',
				public: true,
				enterprise: true,
				modules: ['scalability'],
				invalidValue: '',
				enableQuery: { _id: 'Accounts_StatusVisibility_Enabled', value: true },
				i18nDescription: 'Accounts_StatusVisibility_DefaultRoles_Description',
			});
		});
